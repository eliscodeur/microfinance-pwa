...
@section('content')
    <div class="container-fluid">
        <div class="d-flex justify-content-between align-items-center mb-4">
            ...
            {{-- <div class="d-flex gap-2">
                <a href="{{ route('admin.agents.export', array_merge(request()->query(), ['format' => 'csv'])) }}"
                    class="btn btn-outline-secondary">Exporter CSV</a>
                <a href="{{ route('admin.agents.export', array_merge(request()->query(), ['format' => 'excel'])) }}"
                    class="btn btn-outline-success">Exporter Excel</a>
            </div> --}}
        </div>
                    ...
                    <div class="col-md-10">
                        ...
                        <div class="input-group">
                            <input type="text" name="search" class="form-control"
                                placeholder="Rechercher par nom, code ou téléphone..." value="{{ request('search') }}">
        ...
        <div class="card border-0 shadow-sm mb-4">
            <div class="card-body">
                <div class="table-responsive">
                    <table class="table table-hover align-middle border" id="agentsTable">
                                    ...
                                    @can('Activer/Désactiver')
                                        <td class="text-center">
                                            <button type="button" id="btn-status-{{ $agent->id }}"
                                                ...
                                                <i class="bi {{ $agent->actif ? 'bi-toggle2-on text-success' : 'bi-toggle2-off text-secondary' }}"
                                    ...
                                    @can('Gérer Synchros')
                                        <td class="text-center">
                                            <button type="button" id="btn-sync-{{ $agent->id }}"
                                                ...
                                                <i class="bi {{ $agent->can_sync ? 'bi-cloud-check-fill text-primary' : 'bi-cloud-slash text-muted' }}"
        ...
        <div class="modal-dialog modal-dialog-centered">
            <div class="modal-content border-0 shadow">
                    ...
                    <div class="alert alert-warning small py-3 mt-3 border-0 shadow-sm text-start">
                        <div class="d-flex">
                            ...
                            <div>
                                ...
                                Seuls les agents n'ayant <strong>jamais</strong> eu d'activité (collectes ou
                                clients)
                ...
                <div class="modal-footer justify-content-center border-0 pb-4">
                    ...
                    <button type="button" id="confirmDelBtn" class="btn btn-danger px-4 shadow-sm"
    ...
    <script>
        ...
        function confirmSync(id, currentState, name) {
            ...
            document.getElementById('syncModalMessage').innerHTML =
                `Voulez-vous <strong>${action}</strong> l'accès à la synchro pour <strong>${name}</strong> ?`;
        ...
        async function executeSyncToggle() {
            ...
            try {
                ...
                if (data.success) {
                    ...
                    rowBtn.setAttribute('onclick',
                        `confirmSync(${currentAgentId}, ${isNowSynced}, '${data.agent_name.replace(/'/g, "\\'")}')`);
        ...
        function updateAgentSyncButton(agent) {
            ...
            rowBtn.setAttribute('onclick',
                `confirmSync(${agent.id}, ${isNowSynced}, '${String(agent.nom).replace(/'/g, "\\'")}')`);
        }
                ...
                .then(data => {
                    ...
                    for (const [agentId, canSync] of Object.entries(data)) {
                        ...
                        if (icon) {
                            ...
                            const baseClass = "bi";
                            const statusClass = (canSync == "1") ? "bi-cloud-check-fill text-primary" :
                                "bi-cloud-slash text-muted";
        ...
        $(document).ready(function() {
            $('#agentsTable').DataTable({
                ...
                language: {
                    ...
                    info: "Affichage des agents _START_ à _END_ sur _TOTAL_ agents",
