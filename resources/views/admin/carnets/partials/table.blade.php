<div class="table-responsive">
    <table class="table table-hover align-middle mb-0" id="carnetsTable">
        <thead class="table-light">
            <tr>
                <th class="ps-3 text-start">Numéro</th>
                <th>Client</th>
                <th>Statut</th>
                @if ($type == 'tontine')
                    <th class="text-center">Nombre de cycles</th>
                @else
                    <th>Liaison</th>
                @endif
                <th>État Crédit</th>
        ...
        <tbody>
            @forelse($items as $carnet)
                <tr>
                    <td class="ps-3 fw-bold text-primary text-start">{{ $carnet->numero }}</td>
                    <td>
                    ...
                    <td>
                        ...
                        @else
                            <span
                                class="badge bg-light text-warning border border-warning small">{{ ucfirst($carnet->statut) }}</span>
                        @endif
                    </td>

                    {{-- Colonne dynamique : Cycles (si tontine) OU Liaison (si autre) --}}
                    @if ($type == 'tontine')
                    ...
                    <td class="text-end pe-3">
                        <div class="btn-group">
                            <a href="{{ route('admin.carnets.show', $carnet->ulid) }}"
                                class="btn btn-sm btn-outline-secondary" title="Voir détails">
                                <i class="bi bi-eye"></i>
                            </a>
                            {{-- <button type="button" class="btn btn-sm btn-outline-primary edit-btn"
                                data-id="{{ $carnet->id }}" data-numero="{{ $carnet->numero }}"
                                data-client="{{ $carnet->client_id }}" data-type="{{ $carnet->type }}"
                                data-date="{{ $carnet->date_debut }}" data-parent="{{ $carnet->parent_id }}"
                                title="Modifier">
