...
@section('content')
            ...
            <div class="col-md-4">
                <div class="card border-0 border-start border-success border-4 shadow-sm">
                    <div class="card-body">
                        ...
                        <h3 class="text-success fw-black mb-0">
                            {{ number_format($stats->montant_total_attente, 0, ',', ' ') }} F</h3>
                    </div>
                </div>
            </div>
            <div class="col-md-4">
                                ...
                                <tr>
                                    ...
                                    <td class="ps-4">
                                        <div class="d-flex align-items-center">
                                            <div class="bg-soft-primary rounded-circle p-2 me-3 d-flex align-items-center justify-content-center text-primary fw-bold"
                                                style="width: 40px; height: 40px; background: #eef2ff;">
                                                {{ strtoupper(substr($data->agent->nom, 0, 1)) }}
                                            </div>
                                            <div>
                                                <div class="fw-bold text-dark">{{ strtoupper($data->agent->nom) }}
                                                    {{ $data->agent->prenom }}</div>
                                                <small class="text-muted" style="font-size: 0.7rem;">
                                                    ID: <span
                                                        class="badge bg-light text-dark border-0 p-0">{{ $data->agent->code_agent ?? 'N/A' }}</span>
                                                </small>
                                            </div>
                                        </div>
                                    </td>
                                    ...
                                    <td class="text-center">
                                        <span
                                            class="badge rounded-pill {{ $data->total_commissions > 0 ? 'bg-info-subtle text-info border border-info' : 'bg-light text-muted border' }} px-3">
                                    ...
                                    <td class="text-center">
                                        <span
                                            class="badge rounded-pill {{ $data->total_manuels > 0 ? 'bg-warning-subtle text-warning border border-warning' : 'bg-light text-muted border' }} px-3">
                                    ...
                                    {{-- Total Global --}}
                                    <td class="text-center">
                                        <div class="fw-bolder text-primary" style="font-size: 1.05rem;">
                                            {{ number_format($data->total_global, 0, ',', ' ') }} F</div>
                                        <div class="text-muted" style="font-size: 0.65rem;">{{ $data->nb_items }}
                                            opération(s)</div>
                                    </td>
                                    ...
                                    <td class="text-end pe-4">
                                        <div class="btn-group">
                                            {{-- Détails --}}
                                            <button type="button" class="btn btn-sm btn-outline-primary show-bonus-details"
                                            ...
                                            <form action="{{ route('admin.bonuses.bulk-approve') }}" method="POST"
                                                class="d-inline bulk-approve-form">
                                                @csrf
                                                <input type="hidden" name="agent_id" value="{{ $data->agent_id }}">
                                                <button type="button"
                                                    class="btn btn-success btn-sm px-3 ms-1 bulk-approve-btn"
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            document.querySelectorAll('.confirm-approve').forEach(button => {
                button.addEventListener('click', function() {
                    ...
                    confirmAndSubmit('Valider le paiement ?', '#198754', form.action, null,
                        `Souhaitez-vous confirmer le paiement de ${montant} F ? Cette action créera un reçu officiel.`
                    );
                });
            });
            ...
            document.querySelectorAll('.confirm-reject').forEach(button => {
                button.addEventListener('click', function() {
                    ...
                    confirmAndSubmit('Refuser ce bonus ?', '#d33', form.action, null,
                        "Le bonus sera marqué comme refusé et archivé avec un montant de 0 F. Cette action est irréversible.",
                        'warning');
                });
            });
            ...
            document.querySelectorAll('.bulk-approve-btn').forEach(button => {
                button.addEventListener('click', function() {
                    ...
                    const montant = this.dataset.montant;
                    confirmAndSubmit('Confirmer le paiement global', '#198754', form.action, {
                            agent_id: form.querySelector('[name="agent_id"]').value
                        },
                        `Voulez-vous valider le paiement total de ${montant} F pour cet agent ?`,
                        'info');
                });
            });
            ...
            document.querySelectorAll('.show-bonus-details').forEach(button => {
                button.addEventListener('click', function() {
                    ...
                    let tableRows = items.map(item => {
                        const date = new Date(item.date_attribution).toLocaleDateString(
                            'fr-FR');
        ...
        function executeAction(type, id) {
            const config = {
                approve: {
                    title: 'Valider ce bonus ?',
                    color: '#198754',
                    url: `/admin/bonuses/${id}/approve-single`,
                    icon: 'question'
                },
                reject: {
                    title: 'Refuser ce bonus ?',
                    color: '#d33',
                    url: `/admin/bonuses/reject/${id}`,
                    icon: 'warning'
                }
            };
            const c = config[type];
        ...
        function executeBulkApprove(agentId) {
            confirmAndSubmit('Tout valider et payer ?', '#198754', '/admin/bonuses/bulk-approve', {
                agent_id: agentId
            }, 'Cette action validera tous les bonus en attente pour cet agent.', 'question');
        }
        ...
        function confirmAndSubmit(title, color, url, data = null, text = '', icon = 'question') {
            Swal.fire({
                ...
                borderRadius: '15px',
                backdrop: icon === 'info' || icon === 'question' ? `rgba(25, 135, 84, 0.1)` :
                    `rgba(211, 33, 33, 0.1)`
            }).then((result) => {
        ...
        function ouvrirFormulaireBonus() {
            Swal.fire({
                ...
                <div class="text-start p-2">
                    ...
                    <div class="mb-3">
                        <label class="form-label fw-bold small text-secondary">Montant (F CFA) </label>
                        <input type="number" id="swal-montant" class="form-control rounded-3" min="1">
                    </div>

                    <div class="mb-2">
                        <label class="form-label fw-bold small text-secondary">Motif ou Justification </label>
                        <textarea id="swal-motif" class="form-control rounded-3" rows="3"></textarea>
                    </div>
                </div>
            `,
                showCancelButton: true,
                ...
                preConfirm: () => {
                    ...
                    return {
                        ...
                        date_attribution: new Date().toISOString().split('T')[
                            0] // Optionnel : date du jour AAAA-MM-JJ
                    };
                }
            }).then(async (result) => {
