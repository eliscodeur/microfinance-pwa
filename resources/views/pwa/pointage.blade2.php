...
<script type="module">
    ...
    async function initPage() {
        try {
            ...
            const client = await activeDB.clients.get(Number(carnet.client_id));
    ...
    window.changePointage = (v, max) => {
        ...
        if (max && n > max) {
            Swal.fire({
                ...
                text: `Il ne reste que ${max} pointages.`,
    ...
    window.ouvrirConfirm = async (id, mise) => {
        ...
        const total = nb * mise;
        let jour;
        nb == 1? jour="jour" : jour="jours";
        const result = await Swal.fire({
            title: 'Confirmer le pointage',
            html: `Enregistrer <b>${nb} ${jour}</b> soit <b>${total} FCFA</b> ?`,
            icon: 'question',
    ...
    window.ouvrirCloture = async (id) => {
        const result = await Swal.fire({
            ...
            text: "Cette action est définitive pour ce cycle.",
            icon: 'warning',
            showCancelButton: true,
        ...
        Swal.fire({
            ...
            html: `
                <div class="text-start small">
                    ...
                    <p>Jours pointés: <b>${s.fait} / 31</b></p>
                    <p>Mise journalière: <b>${mise} FCFA</b></p>
                    <hr>
                    <p>Commission: <b class="text-danger">-${comm} FCFA</b></p>
                    <h4 class="text-center mt-3">TOTAL NET :</h4>
                    <h2 class="text-center text-success fw-bold">${net} FCFA</h2>
                </div>
            `,
            icon: 'success',
    ...
    window.creerCycle = async (cid) => {
        ...
        if (existant) {
            Swal.fire('Erreur', 'Un cycle est déjà en cours.', 'error');
            return;
        }
        ...
        while (joursAjoutes < 31) {
            ...
            if (dateFin.getDay() !== 0) joursAjoutes++;
        }
