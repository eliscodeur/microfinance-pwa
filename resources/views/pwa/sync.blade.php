...
@section('header')
    <div class="d-flex align-items-center w-100 bg-white py-1">
        ...
        <div class="d-flex align-items-center justify-content-between flex-grow-1">
            ...
            <span id="network-status-badge"
                class="badge rounded-pill px-3 py-2 shadow-sm border small transition-status animate-fade">
...
@section('content')
    <div class="container-fluid p-0" style="background-color: #f8f9fa; min-height: 100vh;">
        <div class="p-4" style="padding-bottom: 180px !important;">
                ...
                <div class="card-body p-4">
                    ...
                    <div class="sync-list">
                        <div class="d-flex justify-content-between align-items-center py-3 border-bottom">
                            ...
                            <span id="count-cycles"
                                class="badge bg-secondary-subtle text-secondary rounded-pill px-3 py-2 fw-bold shimmer-loading">0</span>
                        </div>
                        <div class="d-flex justify-content-between align-items-center py-3 border-bottom">
                            <span class="text-secondary">Collectes réalisées</span>
                            <span id="count-collectes"
                                class="badge bg-secondary-subtle text-secondary rounded-pill px-3 py-2 fw-bold shimmer-loading">0</span>
                        </div>
            ...
            <div class="mt-5">
                <h6 class="text-uppercase text-muted fw-bold mb-3" style="font-size: 0.75rem; letter-spacing: 1px;">Derniers
                    envois</h6>
                        ...
                        <div class="d-flex align-items-center bg-white p-3 rounded-3 shadow-sm mb-2">
                            ...
                            <div class="flex-grow-1">
                                ...
                                <span class="small text-dark fw-medium">
                                    {{ $batch->nb_collectes }} collectes -
                                    {{ number_format($batch->total_montant, 0, ',', ' ') }} FCFA
                                </span>
                            </div>
                            @if ($batch->status === 'approved')
    ...
    <script type="module">
        ...
        window.rafraichirResumeSync = async function() {
            ...
            if (document.getElementById('count-collectes')) document.getElementById('count-collectes').innerText =
                pColl.length;
            if (document.getElementById('count-cycles')) document.getElementById('count-cycles').innerText = pCyc;
            if (document.getElementById('total-amount')) {
                document.getElementById('total-amount').innerText = new Intl.NumberFormat('fr-FR').format(total) +
                    ' FCFA';
            }
        ...
        window.demarrerAction = async () => {
            ...
            try {
                const authRes = await fetch(
                    `{{ route('pwa.check-sync-permission') }}?matricule=${getMatricule()}&t=${Date.now()}`, {
                        method: 'GET',
        ...
        async function surveillerLeBatch(uuid) {
            ...
            try {
                while (true) {
                    ...
                    const url = `{{ route('pwa.sync-batches.status', ['syncUuid' => ':uuid']) }}`.replace(':uuid',
                    uuid);
        ...
        async function finaliserTout(serverData) {
            ...
            try {
                ...
                if (serverData.data) {
                    await populateDatabase(serverData.data, {
                        replaceAll: true
                    });
                }
        ...
        function savePendingSyncJob(batch) {
            localStorage.setItem(getSyncKey(), JSON.stringify(batch));
        }
        ...
        async function initialiserDonneesSiVide() {
            try {
                ...
                // return
                await populateDatabase(payload, {
                    replaceAll: true
                });
        ...
        async function annulerAttenteBatch() {
            const job = getPendingSyncJob();
            if (job) {
                fetch(`{{ route('pwa.sync-batches.cancel', ['syncUuid' => ':uuid']) }}`.replace(':uuid', job
                    .sync_uuid), {
                        method: 'POST',
                        credentials: 'same-origin',
                        headers: {
