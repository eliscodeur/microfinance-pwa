            ...
            <div class="row text-center align-items-center g-0">
                <div class="col-4 border-end" onclick="goToCollecte()" style="cursor: pointer;">
                    <small class="text-muted d-block mb-1" style="font-size: 0.70rem; white-space: nowrap;">
                        <i class="bi bi-cash-coin text-success me-1"></i>Montant encaissé
                    </small>
                    <span class="fw-bold text-success" id="total-montant" style="font-size: 0.85rem; white-space: nowrap;">0
                        F</span>
                </div>
            ...
            searchTimeout = setTimeout(async () => {
                try {
                    ...
                    if (resultats.length === 0) {
                        area.innerHTML =
                            `<div class="text-center p-5 text-muted small">Aucun résultat pour "${rawQuery}"</div>`;
                        return;
                    }
                    ...
                    resultats.forEach(c => {
                        ...
                        const carnetsHtml = sesCarnets.map(carnet => {
                            const cyclesEnCours = tousCycles.filter(cy => cy
                                .carnet_id === carnet.id && cy.statut === 'en_cours'
                            ).length;
                            const total = cyclesEnCours + (carnet
                                .nombre_cycles_termines || 0);
