    ...
    <div class="d-flex align-items-center w-100 bg-white py-1">
        ...
        <div id="header-search-mode" class="d-flex align-items-center w-100 d-none px-2">
            ...
            <div class="position-relative flex-grow-1">
                <input class="form-control border-0 bg-light pe-5 custom-search-input" list="datalistClientsCycles"
                    id="inputSearchClient" placeholder="Chercher client..."
                    style="border-radius: 8px; font-size: 1rem; height: 45px;"
                    oninput="gererAffichageCroix(); afficherCyclesRegroupes();">

                <button id="btnViderRecherche" onclick="viderRecherche()"
    ...
    <script type="module">
        ...
        async function afficherCyclesRegroupes() {
            ...
            try {
                ...
                let html =
                    `<div class="px-2 mb-2 text-muted small fw-bold text-uppercase" style="font-size:0.6rem;">${val ? val : 'TOUS LES CYCLES'}</div>`;

                let auMoinsUnMatch = false;

                clientsTries.forEach(client => {
                    const matchClient = val ? `${client.nom} ${client.prenom}`.toLowerCase().includes(val
                        .toLowerCase()) : true;
                    ...
                    if (cycles.some(cy => cy.client_id === client.id)) {
                        ...
                        clientCarnets.forEach(carnet => {
                            ...
                            const carnetCycles = cyclesChronologiques.filter(cy => cy.carnet_id ===
                                carnet.id);
                            ...
                            `;
                                ...
                                carnetCycles.forEach((cycle, index) => {
                                    const collectesCycle = collectes.filter(col => col
                                        .cycle_uid === cycle.cycle_uid);

                                    const nbrPointages = collectesCycle.reduce((sum, col) =>
                                        sum + parseInt(col.pointage || 0), 0);
                                    const progression = Math.min(Math.round((nbrPointages /
                                        31) * 100), 100);
                                    const brut = collectesCycle.reduce((sum, col) => sum +
                                        parseFloat(col.montant || 0), 0);
                                    const mise = parseFloat(cycle.montant_journalier || 0);

                                    const estTermine = cycle.statut === 'termine' ||
                                        nbrPointages >= 31;
                                    const estSynchro = cycle.synced === 1;
                                    const afficherSomme = estTermine && (!cycle.retire_at ||
                                        cycle.retire_at === "null");
            ...
            if (result.isConfirmed) {
                try {
                    ...
                    Swal.fire({
                        icon: 'error',
                        title: 'Erreur',
                        text: 'Impossible de supprimer ce cycle.'
                    });
                }
            }
        }
