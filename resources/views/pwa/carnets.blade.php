...
@section('content')
    ...
    //             </div>`;
        ...
        async function initCollectePage() {
            if (!clientId) {
                document.getElementById('collecte-content').innerHTML =
            ...
            try {
                ...
                if (totalCarnets === 0) {
                    container.innerHTML =
                        `<div class="text-center py-5 text-muted">Aucun carnet trouvé pour ce bénéficiaire.</div>`;
                    return;
                }

                const cyclesDuClient = tousLesCycles.filter(cy => Number(cy.client_id) === Number(clientId));
                let html =
                    `<p class="text-muted small mb-3 text-uppercase fw-bold" style="letter-spacing: 1px;">Livrets d'Épargne</p>`;
                ...
                for (const carnet of carnets) {
                    ...
                    const cyclesTerminesNonRetires = cyclesDuCarnet.filter(cy =>
                        cy.statut === 'termine' && (!cy.retire_at || cy.retire_at === "null" || cy.retire_at ===
                            null)
                    );
                    ...
                    const soldeReelDisponible = collectesDesCyclesTermines.reduce((sum, item) => sum + (Number(item
                        .montant) || 0), 0);

                    const cycleActif = cyclesDuCarnet.find(cy => cy.statut === 'en_cours');
                    console.log(cycleActif)
                    const cyclesAencaisser = cyclesDuCarnet.filter(cy => cy.statut === 'termine' && !cy.retire_at);

                    // --- CALCUL RETARD ---
                    let estEnRetard = false;
                    let dateFinAffichable = '---';
                    if (cycleActif && cycleActif.date_fin_prevue) {
                        const dateFinPrevue = new Date(cycleActif.date_fin_prevue);
                        estEnRetard = aujourdhui > dateFinPrevue && cycleActif.statut === 'en_cours';
                        dateFinAffichable = dateFinPrevue.toLocaleDateString('fr-FR');
                    }
                    ...
                    if (cycleActif) {
                        const collectesActives = toutesLesCollectes.filter(col =>
                            String(col.cycle_uid) === String(cycleActif.cycle_uid)
                        );

                        epargneEnCours = collectesActives.reduce((sum, item) => sum + (Number(item.montant) || 0), 0);
                        const nbrPointages = collectesActives.reduce((sum, item) => sum + (Number(item.pointage) || 0),
                            0);
                        pourcentage = Math.min(100, (nbrPointages / 31) * 100);
                    ...
                    html += `
                <div class="card mb-4 shadow-sm border-0 ${estEnRetard ? 'border-danger border-2' : ''}" 
                    style="border-radius: 25px; background: #fff; overflow: hidden; cursor: pointer;"
                    onclick="window.goToPointage(${carnet.id})">
                    
                    <div class="p-3 d-flex justify-content-between align-items-center" style="background: ${estEnRetard ? 'rgba(220, 53, 69, 0.05)' : 'rgba(13, 110, 253, 0.05)'};">
                        <span class="badge rounded-pill ${estEnRetard ? 'bg-danger' : 'bg-primary'} px-3 shadow-sm">LIVRET N° ${carnet.numero || '---'}</span>
                        ${estEnRetard ? '<span class="text-danger fw-bold small"><i class="bi bi-exclamation-triangle-fill"></i> RETARD</span>' : ''}
                        <div class="text-end">
                            <small class="text-muted d-block" style="font-size: 0.65rem; font-weight: 700;">SOLDE DISPONIBLE</small>
                    ...
                    <div class="card-body">
                        ${cycleActif ? `
                                                                                                                            <div class="row align-items-center">
                                                                                                                                <div class="col-7">
                                                                                                                                    <small class="text-muted text-uppercase fw-bold" style="font-size: 0.6rem;">Cycle en cours - Fin : ${dateFinAffichable}</small>
                                                                                                                                    <h3 class="fw-black mb-0" style="color: #2d3436; font-size: 1.8rem;">
                                                                                                                                        ${epargneEnCours.toLocaleString()} <small style="font-size: 0.9rem;">FCFA</small>
                                                                                                                                    </h3>
                                                                                                                                </div>
                                                                                                                            ...
                                                                                                                            <div class="mt-3">
                                                                                                                                <div class="progress" style="height: 10px; border-radius: 10px; background: #f0f2f5;">
                                                                                                                                    <div class="progress-bar ${estEnRetard ? 'bg-danger' : 'bg-primary'} progress-bar-striped progress-bar-animated" style="width: ${pourcentage}%"></div>
                                                                                                                                </div>
                                                                                                                                <div class="d-flex justify-content-between mt-2">
                                                                                                                                    <small class="text-muted" style="font-size: 0.7rem;">Objectif : 31 jours</small>
                                                                                                                                    <small class="fw-bold text-primary" style="font-size: 0.7rem;">${Math.round(pourcentage)}%</small>
                ...
                console.error("Erreur Init Livret:", error);
                document.getElementById('collecte-content').innerHTML =
                    `<div class="alert alert-danger">Erreur lors du traitement local des données.</div>`;
            }
        }
