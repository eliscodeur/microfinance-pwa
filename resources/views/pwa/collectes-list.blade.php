    ...
    <div class="d-flex align-items-center w-100 bg-white py-1">
        ...
        <div id="header-search-mode" class="d-flex align-items-center w-100 d-none px-2">
            ...
            <div class="position-relative flex-grow-1">
                <input class="form-control border-0 bg-light pe-5 custom-search-input" list="datalistClients"
    ...
    <div class="sticky-top bg-white pt-3 pb-2 shadow-sm" style="z-index: 1020; top: 0;">
        <div class="container-fluid px-3">
            <div class="d-flex gap-2" id="filter-group">
                <button onclick="setFiltre('non_synchro')" id="btn-filter-non-synchro"
                    class="btn btn-sm rounded-pill btn-primary shadow-sm flex-fill">
                    <i class="bi bi-cloud-arrow-up"></i> À envoyer
                </button>
                <button onclick="setFiltre('synchro')" id="btn-filter-synchro"
                    class="btn btn-sm rounded-pill btn-outline-secondary flex-fill">
                    <i class="bi bi-cloud-check"></i> Synchro
                </button>
                <button onclick="setFiltre('tous')" id="btn-filter-tous"
                    class="btn btn-sm rounded-pill btn-outline-secondary flex-fill">
                    Toutes
                </button>
            </div>
    ...
    <script type="module">
        ...
        async function chargerDonneesClient() {
            ...
            try {
                ...
                let html =
                    `<div class="px-2 mb-2 text-muted small fw-bold text-uppercase" style="font-size:0.6rem;">${val ? val : 'VUE GLOBALE'}</div>`;
                ...
                for (const clientId in groupement) {
                    ...
                    for (const cyId in cycles) {
                        ...
                        const cycleData = allCycles.find(cy => String(cy.cycle_uid) === String(cyId) || String(cy
                            .id) === String(cyId));
        ...
        async function confirmerSuppression(id) {
            ...
            if (result.isConfirmed) {
                ...
                // return;
                await activeDB.cycles.update(cycleId, {
                    statut: "en_cours"
                });
