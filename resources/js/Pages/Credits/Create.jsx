...
export default function Create({ clients, creditProducts = [] }) {
    ...
    const selectedCarnet = carnets.find(
        (carnet) => String(carnet.id) === String(form.data.carnet_id),
    );
    ...
    const pointageWarning =
        isTontineCarnetSelected &&
        selectedCarnet?.total_pointages < selectedCarnet?.required_pointages;
    ...
    const availableCarnets = useMemo(() => {
        ...
        return client
            ? client.carnets.filter((c) => c.type === form.data.type_support)
            : [];
    ...
    const filteredProducts = useMemo(() => {
        ...
        return creditProducts.filter(
            (p) => p.type_carnet_requis === form.data.type_support,
        );
    ...
    const selectedProduct = useMemo(() => {
        return (
            filteredProducts.find(
                (p) => String(p.id) === String(form.data.credit_product_id),
            ) || null
        );
    ...
    const handleCarnetChange = async (carnetId) => {
        ...
        if (carnetId) {
            const selectedCarnet = availableCarnets.find(
                (c) => String(c.id) === String(carnetId),
            );
            const activeCycleId =
                selectedCarnet?.cycles?.find(
                    (cycle) => cycle.statut === "en_cours",
                )?.id || "";

            try {
                const response = await axios.get(
                    `/admin/credits/check-pending/${carnetId}`,
                );
                ...
                if (draftExists && !response.data.brouillon_bloque) {
                    ...
                    form.setData({
                        ...
                        credit_product_id:
                            response.data.credit_product_id ?? "",
                        ...
                        date_debut: response.data.date_debut
                            ? response.data.date_debut.slice(0, 10)
                        ...
                        nom_prenom:
                            response.data.credit_guarantor?.nom_prenom || "",
                        telephone:
                            response.data.credit_guarantor?.telephone || "",
                        profession:
                            response.data.credit_guarantor?.profession || "",
                ...
                console.error(
                    "Erreur lors de la récupération du brouillon :",
                    error,
                );
                setIsDraftModification(false);
                form.setData({
    ...
    useEffect(() => {
        ...
        if (selectedProduct) {
            form.setData({
                ...
                nombre_echeances:
                    form.data.nombre_echeances ||
                    selectedProduct.duree_max_mois,

                // Gestion du type de crédit selon le support
                type:
                    form.data.type_support === "compte"
                        ? "compte"
                        : form.data.type || "quinzaine",
                periodicite:
            ...
            .then((data) => {
                if (Array.isArray(data)) {
                    ...
                    if (
                        !data.some(
                            (item) =>
                                String(item.id) === String(form.data.carnet_id),
                        )
                    ) {
                        form.setData("carnet_id", "");
                    }
    ...
    const handleTabChange = (targetTab) => {
        ...
        if (
            ["details", "simulation", "resumes", "garanties"].includes(
                targetTab,
            )
        ) {
        ...
        if (["garanties", "resumes"].includes(targetTab)) {
            if (
                ...
                !form.data.montant_demande ||
                form.data.montant_demande <= 0 ||
                !form.data.periodicite ||
                !form.data.nombre_echeances ||
                !form.data.date_debut ||
                !form.data.credit_object_id ||
                !form.data.credit_product_id ||
                form.data.periodicite === "" ||
                form.data.nombre_echeances <= 0
            ) {
                Swal.fire({
                    icon: "warning",
    ...
    const schedule = useMemo(
        () => buildScheduleFromForm(form.data),
        [form.data],
    );
                ...
                <div className="d-flex justify-content-between align-items-center mb-4">
                    <div>
                        ...
                        <p className="text-muted mb-0">
                            Saisie et simulation financière
                        </p>
                    </div>
                    <Link
                        href="/admin/credits"
                ...
                <form onSubmit={submit} className="card shadow-sm border-0">
                    {hasErrors && (
                        <div className="alert alert-danger m-3">
                            ...
                            <strong>Erreur :</strong> Veuillez vérifier les
                            champs soulignés en rouge.
                        </div>
                    )}
                    ...
                    <ul className="nav nav-tabs px-3 pt-3 bg-white border-bottom flex-nowrap overflow-auto shadow-sm">
                        ...
                        <li className="nav-item">
                            <button
                                ...
                                onClick={() =>
                                    handleTabChange("identification")
                                }
                            >
                                <small
                    ...
                    <div className="card-body p-4">
                        ...
                        {activeTab === "identification" && (
                            <fieldset className="mb-2 animate__animated animate__fadeIn">
                                <legend className="text-uppercase h6 text-secondary border-bottom pb-2 mb-3">
                                    1. Identification
                                </legend>
                                <div className="row g-3">
                                    <div className="col-12 mb-3">
                                        ...
                                        <div className="row g-3">
                                            <div className="col-md-6">
                                                <label
                                                    className={`card h-100 border-2 rounded-3 ${form.data.type_support === "compte" ? "border-primary bg-primary bg-opacity-10" : "border-light shadow-sm"}`}
                                                    style={{
                                                        cursor: "pointer",
                                                    }}
                                                >
                                                    ...
                                                    <div className="card-body d-flex align-items-center p-3">
                                                        <div
                                                            className={`rounded-circle p-3 me-3 ${form.data.type_support === "compte" ? "bg-primary text-white" : "bg-light text-muted"}`}
                                                        >
                                                            <i className="bi bi-piggy-bank fs-4"></i>
                                                        </div>
                                                        <div>
                                                            <h6
                                                                className={`mb-0 fw-bold ${form.data.type_support === "compte" ? "text-primary" : "text-dark"}`}
                                                            >
                                                                Compte Épargne
                                                            </h6>
                                            ...
                                            <div className="col-md-6">
                                                <label
                                                    className={`card h-100 border-2 rounded-3 ${form.data.type_support === "tontine" ? "border-primary bg-primary bg-opacity-10" : "border-light shadow-sm"}`}
                                                    style={{
                                                        cursor: "pointer",
                                                    }}
                                                >
                                                    ...
                                                    <div className="card-body d-flex align-items-center p-3">
                                                        <div
                                                            className={`rounded-circle p-3 me-3 ${form.data.type_support === "tontine" ? "bg-primary text-white" : "bg-light text-muted"}`}
                                                        >
                                                            <i className="bi bi-wallet2 fs-4"></i>
                                                        </div>
                                                        <div>
                                                            <h6
                                                                className={`mb-0 fw-bold ${form.data.type_support === "tontine" ? "text-primary" : "text-dark"}`}
                                                            >
                                                                Tontine
                                                            </h6>
                                                            <small className="text-muted">
                                    ...
                                    <div className="col-md-6">
                                        ...
                                        <input
                                            list="clients"
                                            className={`form-control ${form.errors.client_id ? "is-invalid" : ""}`}
                                ...
                                {loadingDetails && (
                                    <div className="text-center py-5">
                                        ...
                                        <p className="text-muted mt-2">
                                            Chargement des détails du carnet...
                                        </p>
                                    </div>
                                )}

                                {carnetDetails &&
                                    carnetDetails.type === "tontine" &&
                                    carnetDetails.cycles && (
                                        ...
                                        <div>
                                            ...
                                            <div className="card border-0 shadow-sm">
                                                ...
                                                <div className="card-body p-0">
                                                    {carnetDetails.historique &&
                                                    carnetDetails.historique
                                                        .length > 0 ? (
                                                        <div className="list-group list-group-flush">
                                                            {carnetDetails.historique.map(
                                                                (
                                                                    transaction,
                                                                    idx,
                                                                ) => (
                                                                    ...
                                                                    >
                                                                        <div className="d-flex align-items-center gap-2 flex-grow-1">
                                                                            ...
                                                                            >
                                                                                <i
                                                                                    className={`bi ${transaction.type_transaction === "Dépôt" ? "bi-arrow-down-short" : "bi-arrow-up-short"} fs-5`}
                                                                                ></i>
                                                                            </div>
                                                                        ...
                                                                        <div className="text-end">
                                                                            <div
                                                                                className={`fw-semibold ${transaction.type_transaction === "Dépôt" ? "text-success" : "text-danger"}`}
                                                                                style={{
                                                                                    fontSize:
                                                                                        "0.85rem",
                                                                                }}
                                ...
                                {!loadingDetails &&
                                    ...
                                    selectedCarnet && (
                                        <div className="alert alert-warning">
                                            ...
                                            Impossible de charger les détails du
                                            carnet.
                                        </div>
                                    )}
                            ...
                            <div className="animate__animated animate__fadeIn">
                                <fieldset className="mb-4">
                                    <legend className="text-uppercase h6 text-primary border-bottom pb-2 mb-3">
                                        <i className="bi bi-sliders me-2"></i>3.
                                        Paramètres financiers &amp; Conditions
                                    </legend>
                                    ...
                                    <div className="row g-3">
                                        ...
                                        <div className="col-md-6">
                                            ...
                                            <select
                                                ...
                                                disabled={
                                                    !form.data.type_support
                                                }
                                                required
                                            >
                                                <option value="" disabled>
                                                    Choisir un produit de
                                                    crédit...
                                                </option>
                        ...
                        {activeTab === "resumes" && (
                            <div className="animate__animated animate__fadeIn">
                                <fieldset className="mb-4">
                                    <legend className="h6 text-uppercase text-secondary border-bottom pb-2 mb-3">
                                        4. Résumés financiers & Échéancier
                                    </legend>
                                ...
                                <div className="d-flex justify-content-between pt-3 border-top mt-4">
                                    ...
                                    <button
                                        ...
                                        className="btn btn-primary px-4"
                                        onClick={() =>
                                            handleTabChange("garanties")
                                        }
                                    >
                                        Étape 5 : Garanties & Pièces
                                        Justificatives{" "}
                                        <i className="bi bi-arrow-right ms-1"></i>
                                    </button>
                                </div>
                        ...
                        {activeTab === "garanties" && (
                            <div className="animate__animated animate__fadeIn">
                                            ...
                                            <div className="card-body p-4">
                                                <fieldset>
                                                    <legend className="text-uppercase h6 text-warning border-bottom pb-2 mb-3 fw-bold">
                                                        ...
                                                        4.1 Caution Solidaire /
                                                        Avaliste
                                                    </legend>
                                                    <p className="text-muted small mb-4">
                                                        Renseignez les
                                                        informations de la
                                                        personne physique qui se
                                                        porte garante du
                                            ...
                                            <div className="card-body p-4">
                                                <fieldset>
                                                    ...
                                                    <p className="text-muted small mb-3">
                                                        Joignez les scans ou
                                                        photos lisibles pour la
                                ...
                                <div className="text-muted small d-none d-lg-block flex-grow-1 text-center px-2">
                                    ...
                                    Tous les champs marqués d'une astérisque (
                                    <span className="text-danger">*</span>) sont
                                    requis pour le comité.
                                </div>
