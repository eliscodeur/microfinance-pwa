...
export function getAgentDB() {
    ...
    db.version(4).stores({
        ...
        agent_stats: "id",
        sync_batches:
            "id, sync_uuid, status, nb_collectes, total_montant, nb_cycles, created_at",
    });
...
export const db = {
    get clients() {
        return getAgentDB()?.clients;
    },
    get carnets() {
        return getAgentDB()?.carnets;
    },
    get cycles() {
        return getAgentDB()?.cycles;
    },
    get collectes() {
        return getAgentDB()?.collectes;
    },
    get agent_stats() {
        return getAgentDB()?.agent_stats;
    },
    get sync_batches() {
        return getAgentDB()?.sync_batches;
    },
        ...
        await database.transaction(
            ...
            async () => {
                ...
                if (data.clients?.length)
                    await database.clients.bulkPut(data.clients);
                if (data.carnets?.length)
                    await database.carnets.bulkPut(data.carnets);
                if (data.sync_batches?.length)
                    await database.sync_batches.bulkPut(data.sync_batches);
                ...
                if (data.cycles?.length) {
                    const normalizedCycles = data.cycles.map((cycle) => {
                        ...
                        const retraitsAssocies = (data.retraits || []).filter(
                            (r) =>
                                r.cycle_uid === cycle.cycle_uid ||
                                Number(r.cycle_id) === Number(cycle.id),
                        );

                        // Calcul de la somme des retraits
                        const sommeRetraits = retraitsAssocies.reduce(
                            (total, r) => total + Number(r.montant),
                            0,
                        );
                        ...
                        return {
                            ...
                            solde_net_final: Number(
                                cycle.solde_restant_net || 0,
                            ),
                        };
                    });
                ...
                if (data.bonus_en_attente) {
                    try {
                        ...
                        if (data.bonus_en_attente.length > 0) {
                            await database.bonus_en_attente.bulkPut(
                                data.bonus_en_attente,
                            );
                        }
                    } catch (error) {}
                }
                ...
                if (data.historique_paiements) {
                    try {
                        ...
                        if (data.historique_paiements.length > 0) {
                            await database.paiements_valides.bulkPut(
                ...
                if (data.stats_performance && data.agent) {
                    try {
                        await database.agent_stats.put({
                            ...
                            volume_historique_global: Number(
                                data.stats_performance
                                    .volume_historique_global || 0,
                            ),
                            total_historique_cycles_termines: Number(
                                data.stats_performance
                                    .total_historique_cycles_termines || 0,
                            ),
                            historique_courbe:
                                data.stats_performance.historique_courbe || [], // Le tableau complet pour le graphique
                        });
                    } catch (error) {}
                }
            },
...
export const DBManager = {
    async searchCarnets(query) {
        ...
        const carnets = await database.carnets
            .filter(
                (car) =>
                    car.numero.toLowerCase().includes(term) ||
                    filteredClientIds.includes(car.client_id),
            )
            .toArray();
    ...
    async getActiveCycle(carnetId) {
        ...
        return database.cycles
            .where("carnet_id")
            .equals(isNaN(carnetId) ? carnetId : Number(carnetId))
